...
<div>
  <div class="heade"  style="margin-left: 225px;margin-right: 225px"
    ><dv-border-box-7 >汽车功率排行榜</dv-border-box-7></div>
    <el-row>
  <el-col :span="16">
  <el-table
...
<script>
  export default {
    ...
    mounted: function () {
      const _this=this
      axios.get('http://180.76.235.163:8088/carguide/toprank?type=4&page='+this.currentPage).then(function(res1){
        console.log(res1)
            for(var i=0;i<res1.data.records.length;i++)
            {
              var power=res1.data.records[i].sysParaPower
              var brandName=res1.data.records[i].sysParaCarSeries
              var id=1+i+(_this.currentPage-1)*_this.pageSize
              var duixiang={id,brandName,power}
    ...
    methods: {
      ...
      const _this=this
      axios.get('http://180.76.235.163:8088/carguide/toprank?type=4&page='+_this.currentPage).then(function(res1){
        console.log(res1)
            for(var i=0;i<res1.data.records.length;i++)
            {
              var power=res1.data.records[i].sysParaPower
